...
resource "docker_image" "imagen_apache" {
    name = "httpd:alpine3.14"
}

resource "docker_container" "contenedor_apache" {
    name = "mi-apache"
    image = docker_image.imagen_apache.latest # Esta propiedad espera el IDENTIFICADOR DE LA IMAGEN 
}
